...
def run(cmd, lock, position, **kwargs):
    ...
    slice_count = round(
        len([img for img in os.listdir(fp) if img.endswith("png")]) / sampling
    )
    ...
    if not kwargs["dryrun"]:
        with subprocess.Popen(
            ...
            if p.returncode is not None and p.returncode > 0:
                logging.error(
                    f"Error encountered. 'rootCrownSegmentation' returned {p.returncode}"
                )
...
def csv(*args, **kwargs):
    ...
    try:
        ...
        dff = df[
            [
                "UID",
                "Predicted Grayscale Image Folder",
                "Lower Bound Threshold Value (uint8)",
                "Upper Bound Threshold Value (uint8)",
                "path_exists",
            ]
        ]
        ...
        for fp in set(
            [
                os.path.dirname(scan["Predicted Grayscale Image Folder"])
        ...
        for fp in validated_scans:
            ...
            ofp = os.path.join(
                thresholded_folder, os.path.basename(gfp)
            )  # output folder for segmented images
            out_fp = os.path.join(
                model_folder, f"{volume_name}.out"
            )  # root system .OUT file
            obj_fp = os.path.join(
                model_folder, f"{volume_name}.obj"
            )  # root system .OBJ file
            soil_out_fp = os.path.join(
                model_folder, f"{volume_name}_soil.out"
            )  # dirt .OUT file
            soil_obj_fp = os.path.join(
                model_folder, f"{volume_name}_soil.obj"
            )  # dirt .OBJ file
        ...
        with ThreadPool(kwargs["threads"]) as p:
            ...
            for i, cmd in enumerate(cmd_list, start=1):
                ...
                p.apply_async(
                    ...
                    error_callback=subprocess_error_callback,
                )
            p.close()
            p.join()
            if not kwargs["verbose"]:
...
def main(args):
    ...
    for fp in args.path:
        ...
        ofp = os.path.join(
            thresholded_folder, os.path.basename(fp)
        )  # output folder for segmented images
        out_fp = os.path.join(
            model_folder, f"{volume_name}.out"
        )  # root system .OUT file
