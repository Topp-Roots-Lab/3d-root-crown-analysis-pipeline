...
int segment(string grayscale_images_directory, int sampling,
            string binary_images_directory, string filepath_out,
            string filepath_obj, int user_threshold_value) {
  // Initialize OUT file
  FILE *Outfp = fopen(filepath_out.c_str(), "w");
  int numVert = 0;
  fprintf(Outfp, "# %s\n", VERSION.c_str());
  ...
  for (int n = 0; n < slice_count; n += sampling) {
    ...
    if (threshold_value > -1) {
      threshold_value = cv::threshold(grayscale_image, binary_image,
                                      user_threshold_value, 255, THRESH_BINARY);
      // Otherwise, determine a threshold value for segmentation
    } else {
      threshold_value =
          threshold(grayscale_image, binary_image, 0, 255, THRESH_TRIANGLE);
    }
  ...
  for (int n = 0; n < fn.size(); n += sampling) {
    ...
    if (count_prev_root > 0 &&
      ...
      if (
          ...
          // 			(there was soil in previous slice AND the percentage of
          // (intersection of current soil and previous root) / (current soil)
          // is less than 70% ) 			 OR
...
int main(int argc, char **argv) {
  try {
    ...
    int threshold_value;       // user-defined grayscale value to use as the
                               // threshold (inclusive)
    ...
    generic.add_options()("help,h", "show this help message and exit")(
        ...
        "cutoff,c", po::value<int>(&threshold_value)->default_value(-1),
        "User defined integer threshold value (inclusive) (default: -1")(
    ...
    if (threshold_value > -1) {
      std::cout << "User-defined threshold value:\t"
                << std::to_string(threshold_value) << std::endl;
    }
    ...
    if (soil_removal_flag) {
      ...
      segment(grayscale_images_directory, sampling, binary_images_directory,
              filepath_out, filepath_obj, threshold_value);
    }
